...
for(irun in 1:nruns) {
        ...
        for(iroute in 1:nroute){
                ...
                log_file = paste0('logs/log-route-', loca_run_sel, '-', route_sel, '.txt')
                ...
                write(control_file, paste0('lib/routectrl.', loca_run_sel, '.', route_sel, '.', data_name, '.inp'))
                runcmd = paste0('./src/rout_f77/rout lib/routectrl.', loca_run_sel, '.', route_sel, '.', data_name, '.inp')
                 # ' > ',  log_file,' 2>&1 &')
                system(runcmd)
        }
}
